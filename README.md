...
Может помочь при:
   - ошибках при скачивании IVI или T-BOX пакетов OTA-обновления, которые блокируют процесс обновления автомобиля
   - ошибках при прошивке IVI или T-BOX, которые блокируют процесс обновления автомобиля
...
1. Подключается к T-Box (`SSH` + `SFTP`/`SCP` fallback).
...
4. Предлагает запустить фикс:
   - если скачаны все пакеты кроме `IVI_MCU`/`IVI_MPU`/`T-BOX`,
   - если произошла ошибка при прошивке `IVI_MCU`/`IVI_MPU`/`T-BOX`.
...
## Требования
   ...
   - Компьютер с WiFi сетью и поддержкой `SSH` (Linux, macOS, Windows).
...
Если у пакета есть `enc`, но нет `otx`, запуск фикса не блокируется. Утилита покажет предупреждение `Warning: missing upgrade spec files (.otx)` — это означает, что OTA-обновление может стартовать не сразу.

Если утилита показывает, что фикс можно запускать `The context.db fix can be started.`, но при этом отсутствуют пакеты `IVI_MCU`/`IVI_MPU`, то их нужно скачать и прошить (до или после запуска фикса):
   - если решаете прошить до запуска фикса, то нужно ввести `no` и выполнить прошивку IVI пакетов, как описано в шаге 6:.
   - если будете прошивать после, то переходите к шагу 7, а шаг 6 выполняйте уже после.
...
Для обновления в состоянии FlashFailure утилита сохраняет существующий `target_baseline_version`.
Если поле пустое, версия извлекается из `release_note_brief` в формате `X.Y.Z` и выводится перед
подтверждением — обязательно перепроверьте её. Если определить версию не удалось, фикс не запускается.
Срок действия такой задачи продлевается на 60 дней, после чего она переводится в состояние ожидания
...
Если утилита помогла вам с OTA-обновлением, сэкономила время или избавила от поездки в сервис, вы можете отблагодарить автора любым удобным вознаграждением:

**[Отправить благодарность](https://pay.cloudtips.ru/p/49a50c39)**

Любая поддержка помогает развивать проект, проверять новые сценарии обновлений и поддерживать инструкцию в актуальном состоянии.

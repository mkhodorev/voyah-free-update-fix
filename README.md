...
2. Скачайте бинарник под вашу ОС:
   - `voyah-free-update-fix-darwin-arm64` для macOS (Apple Silicon),
   - `voyah-free-update-fix-windows-amd64.exe` для Windows x64,
   - `voyah-free-update-fix-linux-amd64` для Linux x64 (если доступен в релизе).
3. Поместите файл в отдельную папку, например `voyah-free-update-fix`.

### Шаг 2. Подготовьте запуск утилиты

Для macOS/Linux:

```bash
chmod +x ./voyah-free-update-fix-darwin-arm64
```

Для Windows:

   - дополнительная подготовка обычно не требуется;
   - если SmartScreen блокирует запуск, нажмите "Подробнее" -> "Выполнить в любом случае" (только если вы уверены в источнике файла).

### Шаг 3. Прошейте T-Box на версию с доступом по SSH
...
### Шаг 4. Подключитесь к WiFi сети автомобиля
...
### Шаг 5. Запустите утилиту voyah-free-update-fix
...
### Шаг 6. Проверьте подключение к T-Box и состояние OTA-обновления
...
### Шаг 8. Запустите фикс для исправления `context.db`
